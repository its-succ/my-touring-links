<script lang="ts">
  ...
  let selectedDepartureDateTime = departureTimes.find((iso) => {
...
<div class="contents">
  ...
  <p class="mdc-typography--body2 text-right">@{touring.sharedBy} が {DateTime.fromJSDate(updatedAt).setZone('Asia/Tokyo')
            .toFormat('MM/dd（EEEEE）HH:mm', { locale: 'ja' })} に共有</p>

  <Select bind:value={selectedDepartureDateTime} label="出発日時">
    {#each departureTimes as departureTime}
      <Option value={departureTime}>{DateTime.fromISO(departureTime)
            .setZone('Asia/Tokyo')
  ...
  ></SharedRouteElement>
    ...
    <p class="mdc-typography--body2 text-right">ルート計算日時: {DateTime.fromISO(touring.touring[selectedDepartureDateTime].calcedAt).setZone('Asia/Tokyo')
            .toFormat('MM/dd（EEEEE）HH:mm', { locale: 'ja' })}</p>
</div>
