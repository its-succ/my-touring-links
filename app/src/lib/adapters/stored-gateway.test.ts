...
describe('StoredGateway<RoutesEntity>', () => {
  it('save', async () => {
    ...
    // action
    const results = await target.save({ name: 'テストルート', routes: routes.toJSON(), userId: 'test-user' });
    // assert
  ...
  it('findById', async () => {
    ...
    routes.addPlaceByDepartureDateTimeToRoutes(now, fakeSpot({ placeId: 'spot1' }));
    routes.addPlaceByDepartureDateTimeToRoutes(now, fakeSpot({ placeId: 'goal' }));
    const entity = { name: 'テストルート', routes: routes.toJSON(), userId: 'test-user',  createdAt: now, updatedAt: now };
